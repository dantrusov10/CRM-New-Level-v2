...
Подробные материалы по текущему состоянию проекта (AS IS) находятся в папке:

- `internal_docs/`

Эта документация предназначена для внутреннего использования (закрытый контур), операционной и продуктовой работы команды.  
Материалы из `internal_docs/` не предназначены для внешней публикации без отдельной редакции.
