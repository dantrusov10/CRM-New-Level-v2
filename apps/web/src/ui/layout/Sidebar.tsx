...
export function Sidebar({ perms }: { perms: PermissionMatrix }) {
  ...
  return (
    <aside className="cockpit-sidebar p-4 overflow-y-auto border-r border-[rgba(51,215,255,0.18)] shadow-[0_0_42px_rgba(45,123,255,0.18)]">
      <div className="mb-5">
        <button
          type="button"
          onClick={() => nav("/dashboard")}
          className="flex items-center gap-3 text-left w-full hover:opacity-95 active:opacity-90 rounded-2xl border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.04)] p-2"
          title="На главную (Dashboard)"
        >
          <img src={logo} alt="NewLevel CRM" className="w-11 h-11 rounded-2xl border border-[rgba(51,215,255,0.35)] shadow-[0_0_24px_rgba(51,215,255,0.22)]" />
          <div>
