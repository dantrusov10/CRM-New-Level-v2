...
export function KpTemplatePresetGallery({
  ...
  function apply(p: KpTemplatePreset) {
    if (!window.confirm(`Применить стиль «${p.name}»? Текущие разделы и оформление будут заменены пресетом.`)) {
      return;
    }
    onApply(applyTemplatePreset(draft, p));
  }
